
const Engine = Matter.Engine;
const World = Matter.World;
const Bodies = Matter.Bodies;
const Body = Matter.Body;
const Constraint = Matter.Constraint;
var bobObject1,bobObject2,bobObject3,bobObject4,bobObject5;
var roofObject;
var rope1,rope2,rope3,rope4,rope5;
function preload()
{
	
}

function setup() {
	createCanvas(800, 700);


	engine = Engine.create();
	world = engine.world;

	//Create the Bodies Here.
	roofObject=new Roof(400,100,400,50)
	bobObject1=new Bob(240,350,40)
	rope1=new Rope(bobObject1.body,roofObject.body,-160,0)
	bobObject2=new Bob(320,350,40)
	rope2=new Rope(bobObject2.body,roofObject.body,-160,0)
	bobObject3=new Bob(400,350,40)
	rope3=new Rope(bobObject3.body,roofObject.body,-160,0)
	bobObject4=new Bob(480,350,40)
	rope4=new Rope(bobObject4.body,roofObject.body,-160,0)
	bobObject5=new Bob(560,350,40)
	rope5=new Rope(bobObject5.body,roofObject.body,-160,0)
	Engine.run(engine);
  
}


function draw() {
  rectMode(CENTER);
  background("cyan");
  roofObject.display();
  bobObject1.display();
  rope1.display();
  bobObject2.display();
  rope2.display();
  bobObject3.display();
  rope3.display();
  bobObject4.display();
  rope4.display();
  bobObject5.display();
  rope5.display();
  
  drawSprites();
 
}



